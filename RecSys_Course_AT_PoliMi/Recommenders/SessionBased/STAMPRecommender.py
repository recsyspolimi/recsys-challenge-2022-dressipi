...
from recbole.model.sequential_recommender.stamp import *
...
OPTIONS = {
    ...
    'data_path': './RecBole_Dataset/STAMP/',
    'dataset': 'dressipi',
...
class STAMPRecommender(BaseRecommender):
    RECOMMENDER_NAME = "STAMPRecommender"
    ...
    def fit(self,
        ...
        self.config = Config(model='STAMP', dataset='dressipi', config_dict=config_dict)
        ...
        self.model = STAMP(self.config, self.train_data.dataset).to(self.config['device'])
        self.logger.info(self.model)
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        stamp_recommendable_scores_batch_internal_ids = self.model.full_sort_predict(interactions)
        # [1:] because first item is score of '[PAD]' (-inf)
        stamp_recommendable_scores_batch_internal_ids = np.array([
            elem.cpu().detach().numpy()[1:] for elem in stamp_recommendable_scores_batch_internal_ids])

        inverse_mapping = self.dataset.id2token(
            self.dataset.iid_field,
            np.arange(1, stamp_recommendable_scores_batch_internal_ids.shape[1] + 1)
        ).astype(int)

        stamp_scores_batch = np.zeros((recommendable_user_id_array.shape[0], self.all_item_id_array.shape[0]))
        stamp_scores_batch[:, inverse_mapping] = stamp_recommendable_scores_batch_internal_ids

        scores_batch = np.zeros((user_id_array.shape[0], self.all_item_id_array.shape[0]))
        scores_batch[recommendable_user_id_mask] = stamp_scores_batch
    ...
    def _build_test_views_df(self):
        stamp_test_sessions_df = self.temp_test_views_df[
            self.temp_test_views_df['item_id'].isin(self.recommendable_item_id_array)]
        test_views_df = pd.DataFrame()
        test_views_df['session_id'] = stamp_test_sessions_df['session_id'].unique()
        # each element of 'item_id_list' column is a list [view1, view2, ...]
        test_views_df['item_id_list'] = stamp_test_sessions_df.groupby('session_id')['item_id'].apply(list).values
