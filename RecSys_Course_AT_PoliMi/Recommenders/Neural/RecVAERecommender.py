...
class RecVAERecommender(BaseRecommender, Incremental_Training_Early_Stopping, BaseTempFolder):
    RECOMMENDER_NAME = "RecVAE"

    def __init__(self, URM_train):
        super(RecVAERecommender, self).__init__(URM_train)
        ...
        def run(dropout, epochs=1):
            for e in range(epochs):
                ...
                for batch in generate(batch_size=self.batch_size, device=self.device,
                    ...
                    #print("{0}: got ratings".format(self.RECOMMENDER_NAME))
                    ...
                    #print("{0}: backward".format(self.RECOMMENDER_NAME))
                    for optimizer in self.optimizers:
                        optimizer.step()
                    #print("{0}: step".format(self.RECOMMENDER_NAME))
                    del ratings
    ...
    def load_model(self, folder_path, file_name=None):
        super(RecVAERecommender, self).load_model(folder_path, file_name=file_name)
