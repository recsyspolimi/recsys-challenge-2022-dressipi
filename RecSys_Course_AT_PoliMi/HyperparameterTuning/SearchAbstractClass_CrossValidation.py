...
class SearchInputRecommenderArgs_CrossValidation(object):
    ...
    def __init__(self,
        ...
        super(SearchInputRecommenderArgs_CrossValidation, self).__init__()
        ...
        if all([isinstance(elem, list) for elem in CONSTRUCTOR_POSITIONAL_ARGS]):
            #  CONSTRUCTOR_POSITIONAL_ARGS is a list of positional args -> model needs to be retrained for each eval
            self.retrain = True
        else:
            self.retrain = False
    ...
    def copy(self):
        ...
        clone_object = SearchInputRecommenderArgs_CrossValidation(
...
class SearchAbstractClass_CrossValidation(object):
    ...
    def __init__(self, recommender_class,
        ...
        super(SearchAbstractClass_CrossValidation, self).__init__()

        self.recommender_class = recommender_class
        self.verbose = verbose
        self.log_file = None
        self.evaluator_validation_list = evaluator_validation_list

        if validation_weights is None:
            self.validation_weights = [1/len(evaluator_validation_list) for _ in range(len(evaluator_validation_list))]
        else:
            self.validation_weights = [elem/sum(validation_weights) for elem in validation_weights]  # normalize weights
    ...
    def _fit_model(self, current_fit_hyperparameters, iteration=None):

        start_time = time.time()

        constructor_positional_args = self.recommender_input_args.CONSTRUCTOR_POSITIONAL_ARGS

        # Construct a new recommender instance
        if iteration is None:
            recommender_instance = self.recommender_class(*constructor_positional_args,
                                                          **self.recommender_input_args.CONSTRUCTOR_KEYWORD_ARGS)
        else:
            recommender_instance = self.recommender_class(*(constructor_positional_args[iteration]),
                                                          **self.recommender_input_args.CONSTRUCTOR_KEYWORD_ARGS)
    ...
    def _evaluate_on_validation(self, current_fit_hyperparameters, was_already_evaluated_flag,
        ...
        if not was_already_evaluated_flag:
            ...
            if not self.recommender_input_args.retrain:
                recommender_instance, train_time = self._fit_model(current_fit_hyperparameters)

            for i in range(len(self.evaluator_validation_list)):
                if self.recommender_input_args.retrain:
                    print("\n\nTraining {}/{}...".format(i + 1, len(self.evaluator_validation_list)))
                    recommender_instance, train_time = self._fit_model(current_fit_hyperparameters, iteration=i)

                print("Evaluating {}/{}...".format(i + 1, len(self.evaluator_validation_list)))
                recommender_instance.set_URM_train(self.evaluator_validation_list[i].URM_test_views)
                result, _ = self.evaluator_validation_list[i].evaluateRecommender(recommender_instance)

                print("MRR value obtained: {}".format(result.loc[100]["MRR"]))
                print("Weight of the validation set: {} %".format(self.validation_weights[i]*100))
                results.append(result)

            result_df = results[0] * self.validation_weights[0]

            n_results = len(results)
            if n_results > 1:
                for i in range(1, n_results):
                    result_df = result_df.add(results[i] * self.validation_weights[i])
